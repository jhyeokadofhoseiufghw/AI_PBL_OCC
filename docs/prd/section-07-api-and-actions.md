...
- 이미지 업로드
- 해당 공연의 기존 게시글 수 확인
- 기존 피드가 없는 경우에만 게시글 생성
- 공연 행 잠금, 트랜잭션 확인, `event_id` 유니크 인덱스로 동시 요청에도 공연당 1개를 초과하지 않음

## `generatePromotionCopy`

- 공연 정보로 5가지 톤의 짧은 피드 문구를 생성
- 선택한 문구는 같은 피드 관리 화면의 본문 입력에 적용
- Timely AI 키는 서버 환경 변수로만 사용
