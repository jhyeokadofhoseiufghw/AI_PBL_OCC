...
- **인증**
  ...
  - 비밀번호/조회 패스워드는 해시 저장
  - Resend와 `occ.pics` 발신 도메인으로 이메일 인증번호 발송
  - 회원가입·이메일 변경·비밀번호 재설정은 서버 액션에서 일회성 인증번호 검증
- **이미지 저장**
...
- 공연별 대표 피드 1개 작성·수정 / AI 문구 생성 / 이미지 붙여넣기와 홈 카드 미리보기 / 홈 통합 피드
...
- 선착순 잔여 수량 초과 예매 방지
- 공연 단일 가격 × 선택 수량이 표시/저장되는지 확인
- 예매 이후 공연 가격이 바뀌어도 기존 예매 금액이 변하지 않는지 확인
- 취소 시 좌석 복구
- 입금 승인 전 QR 접근 불가
- 입장 완료 후 QR 재사용 방지
- 취소 마감 시간 이후 취소 불가
