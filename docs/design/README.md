...
- 여러 QR은 동시에 노출하지 않고 한 장씩 표시하며 좌우 스와이프와 이전/다음 탐색을 제공한다.
...
### O-01·O-02. 회원가입, 로그인과 계정 복구

회원가입 필드: 이메일, 이메일 인증번호, 비밀번호, 이름, 연락처, 단체명.<br>
로그인 필드: 이메일, 비밀번호.
...
- `/forgot-password`에서 가입 이메일 인증 후 비밀번호를 재설정한다.
- 기획자 정보에서 새 이메일 인증 변경과 비밀번호 확인 기반 탈퇴를 제공한다.
- 공개 헤더는 로그아웃 시 `기획자 로그인 / 예매 조회`, 로그인 시 `로그아웃 / 공연 관리`만 표시하며 마지막 버튼을 보라색으로 강조한다.
...
7. 잘못 처리한 티켓은 최근 체크인 목록에서 확인 후 입장 취소 가능
